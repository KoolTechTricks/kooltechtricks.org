...
который также позволяет использовать оффлайн-технологию
[Bergamot](https://browser.mt). В Firefox уже имеется встроенный
оффлайн-переводчик (`about:translations`), но он не поддерживает русский язык.

**Совместимость.** Проблемы с отображением страниц случаются очень редко, в
основном только на каких-то особенных сайтах. О них необходимо сообщать на
...
![Скриншот браузера DuckDuckGo для Android. Справа — окно App Tracking
Protection](/media/browsers_android_duckduckgo.jpg)
...
**Функции.** Содержит стандартные функции: закладки, пароли, виджеты для
главного экрана. Присутствует тревожная кнопка, которая удаляет все данные
сайтов. Сайты можно добавлять в исключения. Отсутствует поддержка веб-приложений
и встроенного просмотра (Custom Tabs).

**Совместимость.** Проблемы с отображением страниц случаются редко, в таких
случаях можно отключить защиту от трекеров для конкретного сайта и сообщить о
проблеме. App Tracking Protection может вызывать сбои, но проблемные приложения
уже отключены.

**Конфиденциальность.** Не сохраняет историю и данные сайтов, можно очистить при
нажатии на кнопку «Огонь». Используется приватный поисковик, но его нельзя
изменить. Блокируются трекеры на сайтах. Можно включить защиту от окон выбора
...
**Производительность (v32.5.2).** Результат теста Speedometer: 17.9 (±0.29).
