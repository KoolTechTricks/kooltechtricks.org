...
вашим почтовым ящиком смотрите [отдельное руководство](/guides/delta-chat-email)
...
Сначала вам нужен любой доступный сервер (релей). Вы можете попросить у
кого-нибудь или найти самостоятельно. Есть [публичный список релеев] (могут быть
недоступны в вашем регионе). Все релеи совместимы между собой: находясь на
одном релее вы можете свободно общаться с пользователями на других релеях.

Когда у вас есть адрес релея, вы можете открыть его в браузере. На сайте будет
QR-код: нажмите на него или отсканируйте, чтобы использовать этот релей в
Delta Chat. Также вы можете просто добавить `/new` к адресу релея, чтобы сразу
...
доходят, то повторите шаги [создания профиля](#создать-профиль) с другим релеем.
...
и мошенничества. Для недоверенных людей можно [создать отдельный профиль](#несколько-профилей-на-одном-устройстве),
...
Поддерживаются [аудио- и видеозвонки](https://delta.chat/ru/help#calls) в личных
чатах. Нажмите на значок телефона в личном чате, чтобы позвонить.
Устанавливается прямое (peer-to-peer) соединение через те же серверы, которые вы
и ваш собеседник используют для текстовых сообщений.

Групповые звонки и демонстрация экрана не поддерживаются, для этого рекомендуем
[Jitsi Meet].
...
[Каналы](https://delta.chat/ru/help#channels) — это инструмент вещания
«один-ко-многим». Администратор может публиковать сообщения всем подписчикам, а
подписчики не видят друг друга. Похожий концепт популярен в Telegram.

Создать канал: **«+» → «Новый канал»**. Введите название, по желанию выберите
изображение и добавьте описание. Далее по ссылке можно пригласить пользователей,
её можно сбросить в любой момент. Пользователи присоединятся, когда вы будете в
сети.
...
может видеть количество подписчиков, просмотров, время прочтения сообщений. Если
вы собираетесь приглашать недоверенных людей в свой канал, то, возможно, стоит
[создать для него отдельный профиль](#несколько-профилей-на-одном-устройстве),
чтобы не раскрывать свою личность и не получать нежелательные сообщения.

Администратор канала может быть только один, и эту роль невозможно передать
другим пользователям. Подписчики могут отправлять одну из пяти разрешённых
реакций на сообщения в канале (👍, 👎, ❤️, 😂, 🙁). Комментарии не поддерживаются.
Новые участники увидят только 10 последних отправленных и закреплённых сообщений,
но вы можете разослать старые, нажав «Отправить повторно» на этих сообщениях.
...
сообщения, а количество подписчиков и просмотров скрыто. Справа — просмотр
канала как администратор. Видно количество подписчиков, просмотры,
кто присоединяется.](channels.webp)
...
В меню вложений доступен пункт **Приложения**, который откроет каталог
мини-приложений. Если он не загружается, вы можете заменить URL-адрес в
настройках (Дополнительные параметры → URL селектора приложений) на
https://wofwca.github.io/webxdc_website/apps/. Альтернативно вы можете открыть
каталог [в браузере](https://webxdc.org/apps).

Мини-приложения являются специальными архивами с запакованными веб-сайтами
(формат `.xdc`). Вы можете скачать их отдельно и отправить как файл, аналогично
изображениям или документам. Собеседники смогут открыть мини-приложение и
начать с ним взаимодействовать, а также добавить на главный экран телефона.
...
Несмотря на отсутствие доступа к внешнему интернету, не рекомендуется
пользоваться мини-приложениями от недоверенных людей. Кто угодно может прислать
поддельную версию, чтобы проще обманом заполучить чувствительные данные.
Мини-приложения в режиме реального времени раскрывают ваш IP-адрес собеседникам.
...
В Delta Chat есть боты, как и в других популярных мессенджерах. Они могут
добавлять взаимодействия с внешним интернетом, связывать пользователей и
автоматизировать процессы. Боты хорошо интегрируются с
[мини-приложениями](#мини-приложения), иногда превращая их в онлайн-сервисы.

Примеры ботов из [каталога](https://deltachat-bot.github.io/public-bots):
- **StickersBot:** Поиск и создание стикеров.
- **FeedsBot:** Подписка на RSS-ленты в групповых чатах.
- **Telegram Connector:** Подписка на крупные телеграм-каналы.
- **PixelSocial:** Маленькая социальная сеть.
...
Соединение происходит по локальной сети, и ему может мешать брандмауэр и VPN,
поэтому если не получается, то отключите.
...
кем связаться. Чтобы этого избежать, добавьте дополнительные релеи.

Процесс добавления релеев идентичен [созданию профиля](#создать-профиль), но
перед этим переключитесь на свой профиль. Будет предложено добавить релей к
вашему профилю. Откроется список подключённых релеев
...
**«Обновление...»**. В меню настроек можно посмотреть подробную информацию.
...
Здесь отображаются добавленные релеи, статус соединения и ошибки. Если вы
