...
Вы можете удостовериться в подлинности приложения, сверив хеш сертификата
подписи с предоставленным разработчиком при помощи приложения [AppVerifier](https://github.com/privacyguides/verified-apps-android#readme).
...
Подключение к интернету (Wi-Fi, мобильные данные), конфигурация Bluetooth и
режим полёта общие для всех профилей.
...
Доверенные центры сертификации устанавливаются для каждого профиля отдельно.
При этом через стандартный интерфейс возможно установить сертификаты только в
профиль владельца и вторичные пользователи. Установка сертификатов в рабочий
профиль потребует стороннее приложение. Попытка установить сертификат в частное
пространство вызывает сбой. [Подробнее](https://infosec.exchange/@kttrickster/117105591253592734)

Большинство приложений используют [прикрепление сертификатов](https://developer.android.com/privacy-and-security/security-config#CertificatePinning)
(SSL pinning). Это значит, что приложения доверяют только тем удостоверяющим
центрам, которые установлены в системе или вшиты в приложение. Даже если вы
добавите сторонний удостоверяющий центр в настройках Android, приложение всё
равно может не доверять ему. Иногда это поведение можно настраивать в приложении.
...
быть заданы индивидуальные способы разблокировки, и они будут защищены
