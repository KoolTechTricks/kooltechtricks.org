...
- На Android и iOS невозможно выдедлить часть текста сообщения.
...
Delta Chat подключается к вашим серверам Chatmail, чтобы отправлять и получать
сообщения. Сервер можно выбрать при создании профиля или добавлении
дополнительных транспортов.

Пользователям присваивается адрес, состоящий из случайных символов и домена
сервера (`u4nfg3gi@example.com`), который могут увидеть ваши контакты. Если вы
решите [сменить основной сервер](/guides/delta-chat#релеи) (и удалить старый),
то новый адрес заменит предыдущий. При этом старые контакты не теряются,
а предыдущие адреса у них всё ещё могут сохраниться.
...
или идентифицировать конкретных людей по одному лишь адресу Chatmail.

Ваш IP-адрес раскрывается:
- Вашим серверам Chatmail.
