...
### Очистка данных сайтов

По умолчанию при закрытии LibreWolf очищаются все данные сайтов, то есть вам
придётся заново войти в аккаунты, как будто вы работаете в режиме инкогнито.
При этом история браузера сохраняется.

Вы можете добавлять сайты в исключения, чтобы их данные не очищались. Нажмите
на значок щита слева в адресной строке и активируйте переключатель
**«LibreWolf: Всегда сохранять куки и данные этого сайта»**.

![LibreWolf: Всегда сохранять куки и данные этого сайта](allow-cookies.webp)

Чтобы посмотреть и отредактировать список разрешённых сайтов, у которых не будут
очищаться данные, зайдите в настройки браузера и раздел «Приватность и защита»
(`about:preferences#privacy`). Ниже в разделе «История» нажмите «Управление исключениями».

![Управление исключениями](manage-exceptions.webp)

Если вам всё же неудобно вести этот белый список сайтов, то можете отключить
очистку данных. В этом же разделе настроек ниже уберите галочку с
...
[WebGL] (Web Graphics Library) используется для аппаратно-ускоренной отрисовки
2D и 3D графики. Часто применяется в играх и интерактивных приложениях. Но также
может использоваться для [уникальной идентификации](https://browserleaks.com/webgl).

Когда сайту требуется WebGL, в адресной строке появляется кнопка, где вы
сможете разрешить эту функцию для конкретного сайта. В настройках LibreWolf
можно включить WebGL глобально, но это не рекомендуется — как правило,
большинство сайтов будут работать и без него.

![Демонстрация разрешения WebGL для конкретного сайта](webgl.webp)
