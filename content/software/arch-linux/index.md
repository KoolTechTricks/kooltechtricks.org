...
предлагают упрощённый вариант установки и имеют специфичные модификации. Они
позволяют получить близкий к Arch Linux опыт. Однако стоит иметь в виду, что
все официальные и большинство сторонних ресурсов про Arch Linux, включая эту
страницу, не предусматривают применение на производных дистрибутивах.

### EndeavourOS

[EndeavourOS](https://endeavouros.com) в народе называют "Arch Linux с
графическим установщиком". Он почти не предлагает крупных модификаций, кроме
опциональной предустановленной темы и некоторых дополнительных пакетов. Этот
дистрибутив самый близкий к Arch Linux, и рекомендуется многим начинающим для
ознакомления.

### CachyOS

[CachyOS](https://cachyos.org) включает многочисленные оптимизации ядра, что
делает операционную систему высокопроизводительной для игр и тяжёлых задач.
Предустановлены графические программы для управления пакетами, ядрами и
конфигурацией, что может быть удобно пользователям, ещё не привыкшим к командной
строке.

### Manjaro

Рекомендацию [Manjaro](https://manjaro.org) можно услышать в качестве простого
варианта Arch Linux. Хотя Manjaro действительно основан на Arch Linux, на деле
далёк от него.
...
Также Manjaro [известен проблемами], которые нередко происходили в прошлом.
Периодически забывают продлевать сертификаты, и однажды из-за ошибки в
собственном пакетном менеджере вызывали DDoS-атаку на AUR.

[известен проблемами]: https://manjarno.pages.dev

### SteamOS

[SteamOS](https://store.steampowered.com/steamos/) предустановлена на всех
игровых устройствах Valve, и с недавних пор может быть установлена на компьютеры
с видеокартами AMD. Однако стоит иметь в виду, что в первую очередь эта
операционная система предназначена для специфичных игровых устройств. Не
поддерживается двойная загрузка с Windows и видеокарты Nvidia.

Valve не рекомендует SteamOS в качестве замены настольной операционной системы.
Возможно перейти в режим рабочего стола с KDE Plasma и работать как за обычным
ПК. Но по умолчанию запускается сеанс Gamescope с клиентом Steam в режиме
Big Picture, что близко к консольному опыту.

SteamOS основана на Arch Linux, но сильно модифицирована. Это неизменяемая
операционная система, которая сбрасывает своё состояние после перезагрузки.
Обновления пакетов могут не обновляться так часто, как в Arch Linux: вы будете
пользоваться более старой версией KDE Plasma.
...
> [!important]
> Перед установкой пакета из AUR следует
[проверять](https://bertptrs.nl/2026/01/30/how-to-review-an-aur-package.html)
содержимое файла `PKGBUILD` и его изменения при обновлении (`git diff master` /
нажать `A`, когда yay попросит показать изменения). Этот файл является
"рецептом" сборки пакета, и может содержать
[вредоносный](https://opennet.ru/63604) [код](https://opennet.ru/65670).
