...
### Проблема официальных прошивок Android
...
Во всех официальных прошивках Android встроены сервисы Google. С их помощью вы
...
Android, в отличие от iOS, более открытая система. Благодаря возможности
устанавливать любые приложения из APK-файлов в обход правил и условий Google
Play, какие-то части системы можно заменить на свободные альтернативы.
Но теперь Google предпринимает шаги по
...
Google в возможности следить за вами и трактовать условия использования
устройства.
...
Смартфоны имеют обширный доступ к данным пользователей. Многие проприетарные
приложения ищут способы отслеживать активность пользователей. Поэтому мобильные
операционные системы iOS и Android ограничивают доступ приложениям к
чувствительным данным. Но всё ещё остаётся много уязвимостей, особенно на
Android, которые могут быть эксплуатированы злоумышленниками.
...
обычных прошивках Android. GrapheneOS даёт больше контроля над тем, как
используются приложения, что позволяет ограничить им сбор данных.

### Сохранение удобства использования

Обычно когда программное обеспечение сосредоточено на безопасности и
приватности, им становится неудобно пользоваться. GrapheneOS в свою очередь
нацелена на всех пользователей, даже менее технически осведомлённых. Поэтому
разработчики стараются сохранять удобство использования на уровне привычных
операционных систем.
...
назад. Начиная с Pixel 8, срок поддержки составляет 7 лет вместо пяти.
...
приватности. Полный перечень на
[официальном сайте](https://grapheneos.org/features). Также смотрите страницу
[руководства по использованию](https://grapheneos.org/usage).
...
виртуальные телефоны. Это отличная возможность изолировать приложения, которые
потенциально могут отслеживать вашу активность. Также для каждого пользователя
можно настроить частные пространства.

Важные особенности:

- Некоторые настройки системы доступны только администратору (Владельцу).
- Настройки сети глобальные для всех пользователей.
- Экземпляры установленных приложений для каждого пользователя одинаковы. Вы не
можете содержать разные версии одного и того же приложения в разных
пользователях.
- В редких случаях приложения могут работать некорректно или не работать вовсе
(например, Termux).
- Локальная сеть не изолируется. Если приложение в одном пользователе заняло
конкретный порт, то приложение в другом пользователе может его прослушать.
    - Это может сломать приложения, которые занимают конкретный локальный порт
    (Например, KDE Connect).
    - На основе этого можно передавать файлы между пользователями с помощью
    приложения [Inter Profile Sharing].
...
- Меньше утечек при использовании VPN. При первом подключении приложения, оно
устанавливается как постоянная VPN, и отключение приводит к отсутствию
интернета.
...
включить. Во время записи видео можно отключить звук. Сканирование QR-кодов
...
звуковых сигналов о начале записи не воспроизводится. Это может быть неэтично
и незаконно в некоторых регионах, вы несёте за это ответственность.
...
Файловый менеджер из AOSP. Он используется в стандартных ОС в качестве
приложения для выбора файлов. Поэтому интерфейс выглядит современным, но
некоторая функциональность отсутствует.
...
запретите им доступ в интернет. Но лучше пробовать свободные клавиатуры,
например, [HeliBoard].
...
изолированный профиль (пользователи, частное пространство), чтобы не давать
всем приложениям доступ к ним.
...
что вынудит пользователей возвращаться на менее безопасные стандартные прошивки
Android.
...
из разных источников, таких как GitHub, F-Droid, RuStore, AppGallery,
APKPure/APKMirror и любых сайтов. При правильном использовании может заменить
собой все другие клиенты магазинов приложений.
...
Скачайте [Sunup] или [другой клиент (дистрибьютор)]. Выдайте клиенту
...
[Sunup]: https://f-droid.org/packages/org.unifiedpush.distributor.sunup
...
Тем не менее значительная часть банков работает исправно. Смотрите также
...
Приложение для оплаты можно установить во вторичном пользователе. Перед оплатой
нужно переключиться на этого пользователя.

Если приложение для оплаты находится в частном пространстве, то вы не сможете
установить его по умолчанию. Перед оплатой нужно открыть приложение. Добавить
ярлык на домашний экран нельзя.
...
Смотрите также [руководство по решению проблем](https://discuss.grapheneos.org/d/8330-app-compatibility-with-grapheneos).
