...
не гонятся за обширной поддержкой устройств. В 2027 году список поддерживаемых
...
вы можете сравнить с [хешем ключа GrapheneOS], чтобы подтвердить целостность
...
[хешем ключа GrapheneOS]: https://grapheneos.org/install/web#verified-boot-key-hash
...
GrapheneOS основана на Android Open Source Project (AOSP). Внешний вид и
пользовательский опыт близок к стоковому Android.

Стоковый Android не содержит огромное разнообразие функций, которое предлагают
оболочки производителей смартфонов. Некоторые функции всё же добираются до
стандартного Android спустя годы, и смартфоны Pixel быстро получают актуальные
версии Android в течении всего срока поддержки, в отличие от смартфонов других
производителей.

Pixel OS — стоковая ОС на Google Pixel, но она имеет незначительные отличия от AOSP.
Как следствие, некоторые функции Pixel OS недоступны в GrapheneOS, но их можно
заменить сторонними приложениями: [Circle to Search], [Tap, Tap]. Кроме того,
...
- Перенос установленных приложений в другие профили.
...
[SeedVault]: https://github.com/seedvault-app/seedvault#readme

### Защита заблокированного устройства

GrapheneOS [хорошо защищает](https://discuss.grapheneos.org/d/40700-grapheneos-protections-against-data-extraction-from-locked-devices)
данные на устройстве от извлечения и кражи, пока оно находится в заблокированном
состоянии. Вдобавок к стандартному шифрованию диска и аппаратной защите, вы
можете воспользоваться следующими функциями:

- Перемешивание кнопок ввода пин-кода.

- Меньше неудачных попыток разблокировки отпечатком пальца, с опциональным
требованием вводить пин-код.

- «Тревожный пароль» (Duress password), при вводе которого все данные устройства
будут стёрты. Это может нести [юридические последствия](https://www.theguardian.com/us-news/2026/jul/23/cop-city-protester-phone).

- Автоматическая перезагрузка после периода неактивности (от 10 минут до 72
часов, по умолчанию 18 часов) и завершение сеансов пользователей. Это выгружает
ключи шифрования из оперативной памяти, что делает невозможным извлечение данных.
...
Ведётся работа над переработкой всех устаревших стандартных приложений.
...
улучшений. Сервисы Google и доступ к интернету не требуются.
...
Встроен блокировщик контента со списками EasyList, но фильтрация рекламы
недостаточно хороша, и её нельзя настроить. Включаются региональные фильтры при
установке соответствующих языков, но реклама на местных сайтах всё ещё может
появляться. Возможно вы захотите установить [другой браузер](/collections/browsers-mobile)
с более хорошей блокировкой контента.
...
Это самый безопасный и неудобный способ просматривать PDF. Отображается только
одна страница на экране, нет поиска по тексту, по ссылкам нельзя перейти,
нельзя рисовать.
...
Поддерживается запись звонков при нажатии на кнопку во время вызова. В
настройках можно включить автоматическую запись для неизвестных номеров и
отдельных контактов. Никаких звуковых сигналов о начале записи для собеседника
не воспроизводится. Это может быть неэтично и незаконно в некоторых регионах,
...
Стандартный магазин приложений «App Store» предназначен для обновления
стандартных приложений GrapheneOS, а также установки некоторых дополнительных:
Accrescent, Android Auto, Google Play Store, сервисы Google Play, Markup
(стандартный редактор изображений в Pixel OS).

Вы можете использовать [Google Play](#google-play) — на GrapheneOS он не имеет
повышенных системных привилегий и изолирован так же, как и обычные приложения.
Accrescent является единственным альтернативным магазином приложений, который
рекомендуется проектом GrapheneOS, однако его пользовательский опыт оставляет
желать лучшего.

Другие приложения могут быть получены из интернета и других магазинов приложений.
Как и любое другое программное обеспечение, оно может являться вредоносным. Хотя
GrapheneOS защищает от эксплойтов, никакая система не может защитить вас от
поддельных и вредоносных приложений, которые крадут личные данные после выдачи
чувствительных разрешений. Поэтому вы должны доверять разработчикам приложений,
которые вы устанавливаете из неизвестных источников.

Мы рекомендуем ознакомиться с [подборкой магазинов приложений для Android](/collections/android-app-stores).
...
Android. Поэтому в долгосрочной перспективе следует обращать внимание и
пробовать [альтернативные магазины приложений](/collections/android-app-stores).
